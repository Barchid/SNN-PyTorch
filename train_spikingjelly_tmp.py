from models.spikingjelly_cnn import CextNet, SpikingJellyNet
import torch
...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    parser.add_argument(
        '-opt', type=str, help='use which optimizer. SDG or Adam')
    ...
    if args.cupy:
        ...
        net = SpikingJellyNet(channels=args.channels)
    print(net)
    net.to(device)
    ...
    train_set = DVS128Gesture(
        args.data_dir, train=True, data_type='frame', split_by='number', frames_number=args.T)
    test_set = DVS128Gesture(args.data_dir, train=False,
                             data_type='frame', split_by='number', frames_number=args.T)
    ...
    out_dir = os.path.join(
        args.out_dir, f'T_{args.T}_b_{args.b}_c_{args.channels}_{args.opt}_lr_{args.lr}_')
    ...
    writer = SummaryWriter(os.path.join(
        out_dir, 'dvsg_logs'), purge_step=start_epoch)
