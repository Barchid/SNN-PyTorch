...
def __gen_ST(N, T, rate, mode="regular"):
    if mode == "regular":
        ...
        spikes = np.ones([T, N])
        spikes[
            np.random.binomial(1, float(1000.0 - rate) / 1000, size=(T, N)).astype(
                "bool"
            )
        ] = 0
        return spikes
...
def image2spiketrain(
    x, y, gain=50, min_duration=None, max_duration=500, input_shape=[28, 28, 1], class_num=10
):
    y = to_one_hot(y, class_num)
    ...
    for i in range(batch_size):
        st = spiketrains(T=T[i], N=Nin, rates=gain * x[i].reshape(-1)).astype(
            np.float32
        )
    ...
    alltgt = np.zeros([max_duration, batch_size, class_num], dtype=np.float32)
    for i in range(batch_size):
        alltgt[:, i, :] = y[i]
...
def target_convolve(tgt, alpha=8, alphas=5):
    ...
    for i in range(tgt.shape[1]):
        for j in range(tgt.shape[2]):
            tmp = np.convolve(np.convolve(tgt[:, i, j], kernel_alpha), kernel_alphas)[
                :max_duration
            ]
            tgt[:, i, j] = tmp
    return tgt / tgt.max()
...
def dynaload(
    ...
    **loader_kwargs
    ...
    loader = DL(
        dataset=dataset, batch_size=batch_size, shuffle=dataset.train, **loader_kwargs
    )
...
def mnist_loader_dynamic(config, train, pre_processed=True, Nparts=1, part=1):
    ...
    dataset = datasets.MNIST(
        root=config.data_path, download=True, transform=transform, train=train
    )
    if Nparts > 1:
...
def get_mnist_loader(
    ...
    **loader_kwargs
    ...
    config = datasetConfig(
        image_size=[28, 28], batch_size=batch_size, data_path="./data/mnist"
    )
